...
class ResultController extends Controller
{
	...
	public function postresult(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			$class = DB::table('Class')->select('*')->where('id', '=', $request->input('class_id'))->first();

			$subGradeing = Subject::select('gradeSystem')->where('code', $request->input('subject_code'))->where('class', '=', $class->code)->first();
			...
			$isAddbefore = Marks::where('regiNo', '=', $regiNos)->where('exam', '=', $request->input('exam_id'))->where('subject', '=', $request->input('subject_code'))->first();

			if ($isAddbefore) {
			} else {
				$marks = new Marks;
				$marks->class = $class->code;
				$marks->section = $request->input('section_id');
				$marks->shift = 'Morning';
				$marks->session = trim($request->input('session'));
				$marks->regiNo = $regiNos;
				$marks->exam = $request->input('exam_id');
				$marks->subject = $request->input('subject_code');
				$marks->written = $writtens;
				$marks->mcq = $mcqs;
				$marks->practical = $practicals;
				$marks->ca = $cas;
				$isExcludeClass = $request->input('class_id');
	...
	public function putresult(Request $request, $result_id)
	{
		$rules = [
			'class_id' => 'required',
			'section_id' => 'required',
