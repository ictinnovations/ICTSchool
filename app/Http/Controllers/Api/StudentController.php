...
class StudentController extends Controller
{
    ...
    public function update_student(Request $request, $student_id)
    {
        ...
        if ($validator->fails()) {
            ...
            $student = Student::select(
                ...
                'gender',
                'religion'
            )->where('Student.isActive', 'Yes')->where('Student.id', $student_id)->first();
    ...
    public function studentnotification(Request $request, $student_id)
    {
        $rules = [
            'name'    => 'required',
            'type'    => 'required',
        ...
        if ($validator->fails()) {
            ...
            if (!empty($ictcore_integration) && $ictcore_integration->ictcore_url != '' && $ictcore_integration->ictcore_user != '' && $ictcore_integration->ictcore_password != '') {
                ...
                return $postmethod->postnotificationmethod($request->input('name'), $request->input('type'), $request->input('message'), 'single', $contact_id);
         ...
         return response()->json($validator->errors(), 422);
                    ...
                    // $headers = apache_request_headers();
                     //dd($headers['Authorization']);
                      ...
                      file_put_contents(public_path('recording/').$filename.'.wav', base64_decode($request->input('recording')));
