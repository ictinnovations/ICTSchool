...
class AttendanceController extends Controller
{
	...
	public function attendance_create(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			//return Carbon::parse($request->input('date'))->format('Y-m-d') ; 
			if (request()->getHttpHost() == 'localhost' || request()->getHttpHost() == 'school.ictcore.org') {
				//$attendance =  Attendance::where('date',Carbon::parse($request->input('date'))->format('Y-m-d'))->delete();
			}
	...
	public function update_attendance(Request $request, $attendance_id)
	{
		...
		if ($validator->fails()) {
			...
			if ($status == 'Absent' || $status == 'absent') {
				...
				$attendance->date = Carbon::parse($request->input('date'))->format('Y-m-d');
				$attendance->regiNo = $students;
				$attendance->status = $status;
				if ($request->input('comments') != '') {
					$attendance->coments = $request->input('comments');
				}
