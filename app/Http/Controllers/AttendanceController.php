...
class attendanceController extends BaseController
{
	...
	public  function printlist(Request $request, $class, $section, $shift, $session, $date)
	{
		if ($class != "" && $section != "" && $shift != "" && $date) {
			...
			$attendance = DB::table('Student')
				...
				->Where('Student.shift', '=', 'Morning')
				->where('Student.session', '=', trim($session))
				->where('Student.isActive', '=', 'Yes')
				->where('Attendance.date', '=', $date)
			...
			$date = $this->parseAppDate($date);
			$fileName = $className->name . '-' . $section . '-' . $shift . '-' . $section . '-' . $date . '.xlsx';

			if (!$attendance->isEmpty()) {
				return Excel::download(new AttendanceExport($input, $attendance), $fileName);
			}
	...
	public function stdatdreportindex()
	{
		...
		return View('app.studentAttendancereprt');
	}
