...
class classController extends BaseController
{
	...
	public function update(Request $request)
	{
		...
		$validator = \Validator::make($request->all(), $rules);
		if ($validator->fails()) {
			return Redirect::to('/class/edit/' . $request->input('id'))->withErrors($validator);
		} else {
			$class = ClassModel::find($request->input('id'));
			$class->name = $request->input('name');
	...
	public function getForsectionjoin($class)
	{
		...
		$sections        = SectionModel::/*join('timetable','section.id','=','timetable.section_id')*/select('section.id', 'section.name')->where('section.class_code', '=', $class)->whereIn('id', $sections)->get();
		$output  = '';
