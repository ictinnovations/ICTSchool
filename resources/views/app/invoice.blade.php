        ...
        <div class="box col-md-12">
            <div class="box-inner">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group ">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" value="{{ $year }}" id="year" required="true"
                                                class="form-control datepicker2" name="year" data-date-format="yyyy">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                ...
                <input value="{{ get_current_session()->id }}" type="hidden" id="session" required="true"
                    class="form-control datepicker2" name="session" data-date-format="yyyy">
                        ...
                        <table id="feeList" class="table table-striped table-bordered table-hover">
                            ...
                            <tbody id="ajax_data">
                                @foreach ($fees as $fee)
                                    ...
                                    ?>
                                    <tr>
                                        <td><a class="btnbill" href="#"
                                                onclick="invoicepaidhistory('{{ $fee->billNo }}');">{{ $fee->billNo }}</a>
                                        </td>
                                        ...
                                        <td>
                                            @if ($fee->title == 'monthly')
                                                {{ \DateTime::createFromFormat('!m', $fee->month)->format('F') }}
                                            @endif
                                        </td>
                                        <td>{{ $fee->date }}</td>
                                        <?php
                                        if ($fee->payableAmount == $fee->adjusted || $fee->adjusted >= $fee->payableAmount) {
                                            $status = 'adjust';
                                        } elseif ($fee->payableAmount === $fee->paidAmount || $fee->paidAmount >= $fee->payableAmount) {
                                            $status = 'paid';
                                        ...
                                        <td>
                                            ...
                                            <button value="{{ $fee->billNo }}" title='Collect Invoice'
                                                class='btn btn-primary @if ($fee->paidAmount < $fee->payableAmount) btninvoice @endif'
                                                @if ($fee->payableAmount === $fee->paidAmount || $fee->paidAmount >= $fee->payableAmount) onclick = "alert('Invoice Fully Paid')" @endif>
                                                <i class="fas fa-dollar-sign icon-white"></i></button>

                                            {{-- <a title='Edit' class='btn btn-warning'
                                                href='{{ url('/fees/invoice/adjust') }}/{{ $fee->billNo }}'> <i
                                                    class="glyphicon glyphicon-pencil icon-white"></i></a> --}}
                                            @php
                                                $feeId = $f_idd->id ?? null;
                                            @endphp
                                            <a title='get vouchar' class='btn btn-info'
                                                href='{{ url("/fee/get_vouchar?class=$fee->class&section=$fee->section&session=jan-2019,march-2019&type=Monthly&month=$fee->month&fee_name=$feeId&regiNo=$fee->regiNo") }}'>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function feecollectionadjust() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('/fees/invoice/adjust/') }}" + '/' + billNo,
                data: {
                    collectionAmount: collectionAmount,
                    payableAmount: payableAmount
                },
                ...
                success: function(data) {
                    ...
                    $.each(data, function(i, student) {
                        ...
                        if (student.regiNo == stdRegiNo) {
                            var opt = "<option value='" + student.regiNo + "' selected>" + student
                                .firstName + " " + student.middleName + " " + student.lastName + "[" +
                                student.rollNo + "] </option>"
                        } else {
                            var opt = "<option value='" + student.regiNo + "'>" + student.firstName +
